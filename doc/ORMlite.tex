...
La classe Database que j'ai créée sert également de base pour toutes les classes devant se trouver dans la base de données. Elle définit une méthode getDao() qui crée automatiquement le bon DAO pour la sous-classe et elle étend BaseDaoEnabled (une classe de ORMlite), ce qui permet d'utiliser directement les objets comme DAOs.
