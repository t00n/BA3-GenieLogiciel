...
\subsection{Test}
Dans le dossier test, j'ai rajouté 3 fichiers qui permettent de tester si la base de données et l'ORM fonctionnent. Lancez simplement TestMain et dites-moi si vous avez des erreurs. Sinon, c'est que ca fonctionne.
...
Dao<Test, Integer> daoTest = new DaoManager.createDao(connectionSource, Test.class);
\end{lstlisting}
...
La doc : 
\begin{itemize}
	\item \url{http://ormlite.com/javadoc/ormlite-core/doc-files/ormlite_2.html#DAO-Setup}
...
Attention, il y a une subtilité : lorsqu'on crée les tables avec TableUtils (section précédente), il faut les créer dans le bon ordre pour ne pas référencer une table qui n'existe pas encore !
Dans l'exemple, il faut créer d'abord OtherTest et puis Test puisque Test référence OtherTest.
...
\begin{itemize}
	...
	\item Créer des query personnalisées pour stocker les .obj en BLOB
\end{itemize}
